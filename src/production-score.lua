...
local function get_product_list()
      ...
      for k, product in pairs (products) do
        ...
        if product_amount > 0 then
          for j, ingredient in pairs (ingredients) do
            recipe.ingredients[ingredient.name] = ((ingredient.amount)/#products) / product_amount
          end
          recipe.ingredients.energy = (recipe_prototype.energy / #products) / product_amount
          table.insert(product_list[product.name], recipe)
        end
      end
    end
  end
  ...
  for k, entity in pairs (entities) do
    if entity.type == "rocket-silo" and entity.fixed_recipe then
      ...
      local recipe_data = { ingredients = {}, silo_name = k }
      for k, product in pairs(recipe.products) do
        local product_amount = util.product_amount(product)
        if product_amount > 0 then
          product_amount = product_amount * required_parts
          recipe_data.ingredients[product.name] = product_amount
        end
      end
      recipe_data.ingredients.energy = recipe.energy
      table.insert(rocket_silos, recipe_data)
    end
  end
      ...
      for k, launch_product in pairs (launch_products) do
        ...
        if launch_product_amount > 0 then
          for k, silo_products in pairs (rocket_silos) do
            ...
            for product_name, product_count in pairs (silo_products.ingredients) do
              this_silo.ingredients[product_name] = product_count / launch_product_amount
            end
            this_silo.ingredients[item.name] = 1 / launch_product.amount
  ...
  for name, recipes in pairs (nil_prices) do
    if #recipes > 0 then
      ...
      for k, recipe in pairs (recipes) do
        ...
        local reverse_price = (product_value - param.energy_addition(energy, recipe, product_value)) / ingredient_multiplier(recipe.ingredients, param) -- Not perfect, but close enough
...
local default_energy_addition = function(recipe, cost)
  return ((ln(recipe.ingredients.energy + 1) * (cost ^ 0.5)))
end
...
production_score.generate_price_list = function(param)
  ...
  get_price_recursive = function(name, current_loop, loop_force)
    ...
    for k, recipe in pairs (entry) do
        if not recipe.ingredients.energy then error(serpent.line(recipe)) end
      local this_recipe_cost = 0
      for ingredient_name, cost in pairs (recipe.ingredients) do
      ...
      if this_recipe_cost > 0 then
        this_recipe_cost = (this_recipe_cost * ingredient_multiplier(recipe.ingredients, param)) + param.energy_addition(recipe.ingredients.energy, recipe, this_recipe_cost)
        if recipe_cost then
